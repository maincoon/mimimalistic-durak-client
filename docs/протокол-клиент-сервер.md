...
participant "Прокси" as Proxy
participant "Микросервисы / Игры" as Games
...
Управление состоянием ожидания хода.

* `wait` — сервер ждет действия от игрока `box` (или `uid`).
* `timeout` — время на раздумья вышло, может идти вместе с `timeBank`.
